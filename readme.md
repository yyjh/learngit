...
#### 普通工作流程
```
1.git跟踪并管理的是修改而非文件，修改必须先add到暂存区(stage)后才能被commit
2.git add readme.md
3.git add .  // add所有文件
...
3.git add .  // 将全部文件加入git版本管理 .的意思是将当前文件夹下的全部文件放到版本管理中
...
4.git branch --set-upstream-to=origin/master master  // 将当前分支设置为远程仓库的master分支
5.git add .  // 将全部文件加入git版本管理 .的意思是将当前文件夹下的全部文件放到版本管理中
6.git commit -m "注释"  // 提交文件 使用-m 编写注释
7.git push  // 推送到远程分支
```
